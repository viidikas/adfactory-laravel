/* AD.FACTORY — mock data (window.AF) ------------------------------------- */
(function () {
  const THUMBS = [
    '#2e6b57', '#365b7a', '#7a5a36', '#6b3550', '#41506b',
    '#5a6b35', '#356b6b', '#6b4135', '#4b3a6b', '#2e5b6b',
    '#6b6235', '#553a2e',
  ];
  const t = (i) => THUMBS[i % THUMBS.length];
  ...
  const BRANDS = ['Monefit SmartSaver', 'Monefit Credit', 'Credit24', 'Creditstar'];

  const CATEGORIES = ['Hook', 'Lifestyle', 'Product', 'Testimonial', 'CTA', 'B-roll'];
  ...
  const clips = clipDefs.map((c, i) => ({
    id: 'CLP-' + String(1042 + i),
    name: c[0],
    category: c[1],
    duration: c[2],
    aspect: c[3],
    tags: c[4],
    color: t(i),
    market: MARKETS[i % MARKETS.length].code,
    resolution: c[3] === '16:9' ? '1920×1080' : c[3] === '1:1' ? '1080×1080' : '1080×1920',
    addedDays: (i * 3 + 2),
    usedCount: ((i * 7 + 3) % 19),
    fav: i % 5 === 0,
  }));

  // ---- Ad designs / templates ---------------------------------------------
  const designs = [
  ...
  // ---- Orders --------------------------------------------------------------
  const STATUSES = ['Draft', 'Submitted', 'In production', 'Rendering', 'Review', 'Delivered'];
  const orderDefs = [
  ...
  const orders = orderDefs.map((o, i) => {
    const status = o[3];
    const stIdx = STATUSES.indexOf(status);
    return {
      id: 'AF-' + String(2401 + i),
      title: o[0],
      brand: o[1],
      ...
      requestedBy: o[6],
      ageDays: o[7],
      progress: Math.min(100, stIdx * 18 + (i % 4) * 4),
      aspect: (designs.find(d => d.id === o[4]) || {}).aspect || '9:16',
      dueDays: [2, 5, 1, 8, -3, 12, 3, 4, -10][i],
    };
  });

  // ---- Copy mapping --------------------------------------------------------
  // maps a copy "slot" to localized variants used by the Templater CSV export
  const copyRows = [
  ...
  window.AF = {
    MARKETS, BRANDS, CATEGORIES, STATUSES,
    clips, designs, orders, copyRows, stats, activity,
    marketOf: (code) => MARKETS.find(m => m.code === code) || { code, flag: '🏳️', name: code },
    designOf: (id) => designs.find(d => d.id === id),
    LANGS: ['EN', 'ES', 'DE', 'EE', 'FR'],
  };
})();
