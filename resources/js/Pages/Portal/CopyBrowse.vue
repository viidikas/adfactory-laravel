...
function reset() { step.value = 1; chosen.value = null; picked.value = []; selLangs.value = ['EN']; selDesigns.value = []; cat.value = 'All'; }
      ...
      <template v-else-if="step === 2">
        ...
        <div v-else :style="{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(var(--grid-min), 1fr))', gap: 'var(--gap)' }">
          <div v-for="c in stepClips" :key="c.id">
            <Thumb :clip="thumb(c)" :selected="picked.includes(c.id)" :show-play="false" @click="togglePick(c.id)" />
