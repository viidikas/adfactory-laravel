...
import {
  ...
  ALL_CATEGORIES, ALL_LANGS, ALL_BRANDS, defaultFilters, DEFAULT_VISIBLE_COLS,
...
onMounted(async () => {
  try {
    const [c, cfg] = await Promise.all([api.get('/api/clips'), api.get('/api/config')]);
    clips.value = Array.isArray(c) ? c : [];
    tstate.value = buildTemplaterState(cfg || {}, clips.value);
...
function build() {
  ...
  try {
    ...
    flash(`Generated ${rows.value.length} rows.`);
  } catch (e) {
...
function exportCsv() {
  ...
  a.download = `adfactory_${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
    ...
    <div :style="{ padding: 'var(--pad-screen)', display: 'flex', flexDirection: 'column', gap: 'var(--gap)' }">
      <div :style="{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '14px' }">
        <div>
          ...
          <p :style="{ color: 'var(--text-2)', margin: '6px 0 0', fontSize: '14.5px' }">Build the Templater CSV — clips × designs × formats × languages × brands, only where copy exists.</p>
        </div>
        <Button variant="ghost" icon-right="arrowright" @click="router.visit('/preview')">Full preview & export</Button>
      </div>
          ...
          <div :style="{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }">
            <div>
              ...
              <div :style="{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }">
                <Tag v-for="b in ALL_BRANDS" :key="b" :active="isOn('brand', b)" @click="toggle('brand', b)">{{ b }}</Tag>
              </div>
              <SectionLabel>Language</SectionLabel>
              <div :style="{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }">
                <Tag v-for="l in ALL_LANGS" :key="l" :active="isOn('lang', l)" @click="toggle('lang', l)">{{ l }}</Tag>
              </div>
        ...
        <Card>
          <div :style="{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '14px' }">
            <div :style="{ fontSize: '14px', color: 'var(--text-2)' }">
              ...
              <span :style="{ color: 'var(--text-1)', fontWeight: 700 }">{{ filters.brand.length }}</span> brand ≈
              <span :style="{ color: 'var(--accent)', fontWeight: 800 }">{{ estimate.total }}</span> rows
            </div>
            <div :style="{ display: 'flex', gap: '10px' }">
              <Button icon="zap" :disabled="building" @click="build">{{ building ? 'Building…' : 'Generate' }}</Button>
